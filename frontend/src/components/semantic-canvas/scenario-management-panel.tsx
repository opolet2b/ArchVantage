 ...
 *
 * Reusable component for managing scenarios (Vertical Modes).
 * Allows creating, editing, and deleting scenarios.
 * Can be used as a full page or embedded in a dialog.
 */
"use client";
...
export function ScenarioManagementPanel({ embedded = false }: ScenarioManagementPanelProps) {
    ...
    const handleSaveScenario = async (data: Partial<Scenario>) => {
        ...
        try {
            ...
            console.error(error);
            const msg = error.message === "Unauthorized" ? "Session invalid" : error.message;
    ...
    if (viewMode === "create" || (viewMode === "edit" && selectedScenario)) {
        return (
            <div className={`h-full w-full mx-auto ${embedded ? 'p-0' : 'p-6 pt-2 max-w-[95vw]'}`}>
                <ScenarioEditor
    ...
    return (
        <div className={`h-full flex flex-col gap-6 w-full mx-auto ${embedded ? 'p-0' : 'p-6 max-w-7xl'}`}>
            ...
            {!embedded && <div className="flex justify-end mb-4"><Button onClick={handleStartCreate}><Plus className="w-4 h-4 mr-2" />New Scenario</Button></div>}


            <div className={`flex-1 flex gap-6 overflow-hidden border rounded-xl bg-card shadow-sm ${embedded ? 'border-none shadow-none' : ''}`}>
                {/* List Sidebar */}
